/* ----------------------------------------------------
* TRUST-PPG 2 on Empatica data (Accelerometer FSR = +- 16g)
* Pipeline: 
...
#include "TRUST_PPG2_empHeartRate.h"
...
// Funzione sigmoid
float sigmoid_conf_trust_ppg2_emp(float x, float midpoint, float scale) {
...
if (values_win_C_File)
    {
        ...
        float c1_sigmoid = sigmoid_conf_trust_ppg2_emp(c1, MIDPOINT_C1, SCALE_C1);
        float c2_sigmoid = sigmoid_conf_trust_ppg2_emp(c2, MIDPOINT_C2, SCALE_C2);
        ...
        // Kalman Filter 1 
        kalman_HR_estimation_float2(HR_meas, state, coeff_sigmoid, &HR_freq_est2, &P);
...
int trust_ppg2_emp_heartrate(time_delta_ms_t delta_ms, ppg_t ppg, accel_t accx, accel_t accy, accel_t accz){
    ...
    if(delta_ms < 60){
        HR_results[0] = main_algorithm_trust_ppg2_emp(delta_ms,ppg,accx,accy,accz);
        ...
        HR_results[0] = main_algorithm_trust_ppg2_emp(delta_ms_interp, ppg_interp, accx_interp, accy_interp, accz_interp);
        HR_results[1] = main_algorithm_trust_ppg2_emp(delta_ms,ppg,accx,accy,accz);
        ...
        HR_results[0] = main_algorithm_trust_ppg2_emp(delta_ms_interp1, ppg_interp1, accx_interp1, accy_interp1, accz_interp1);
        HR_results[1] = main_algorithm_trust_ppg2_emp(delta_ms_interp2, ppg_interp2, accx_interp2, accy_interp2, accz_interp2);
        HR_results[2] = main_algorithm_trust_ppg2_emp(delta_ms,ppg,accx,accy,accz);
