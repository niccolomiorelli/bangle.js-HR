/* ----------------------------------------------------
* TRUST-PPG 4 on Empatica data (Accelerometer FSR = +- 16g)
* Pipeline: 
* - Pre-processing: Bandpass filtering + z-score standardization of the PPG and ACC signals + NLMS adaptive filtering for motion artefacts removal
* - State Machine for motion level detection: Multi-States (Stationary, Transition, Motion Low, Motion Medium, Motion High)
* - FFT-based spectral analysis (on the original PPG or on the NLMS filter output, based on the motion level)
* - Peak selection with motion-based range reduction: BASED ON FIXED RANGES DEPENDENT ON THE MOTION LEVEL
* - Confidence evaluation of the selected peak based on 3 coefficients: 1) Peak power to total power ratio, 2) Peak power to local power ratio, 3) Consistency with the previous peaks. The confidence is then evaluated through a weighted combination of these three coefficients through sigmoid functions.
* - Kalman Filter-based spectral tracking with motion-based process noise and confidence-based measurement noise: DOUBLE-MEASURE KALMAN FILTER
* - Final HR estimation provided in bpm*10
*/
...
#include "TRUST_PPG4_empHeartRate.h"
...
// State Machine
#define THRESHOLD_MOTION1 400.0f
#define THRESHOLD_MOTION2 600.0f
#define THRESHOLD_MOTION3 4000.0f
...
#define MU 0.2f                        // Learning rate of the NLMS filter
...
#define SCALE_C1 10.0f                  // Scale factpr Sigmoid c1
#define SCALE_C2 10.0f                  // Scale factpr Sigmoid c2
#define MIDPOINT_C1 0.35f               // Midpoint Sigmoid c1
#define MIDPOINT_C2 0.65f               // Midpoint Sigmoid c2
#define ALPHA 0.45f                     // Weight for c1
#define BETA  0.45f                     // Weight for c2
#define GAMMA 0.10f                     // Weight for c3
...
#define DUMP_RESULTS_ST_FILE_NAME "bangle.js-HR/Dump_files/list_results_C_float.csv"       // To save the results (Heart rate)
...
static float signal_PPG_buffer[WINDOW_LEN] = {0.0};                                 // Buffer for the standardized PPG signal
static float signal_NLMS_buffer[WINDOW_LEN] = {0.0};                                // Buffer for the output of NLMS filter
...
// State Machine
static int state = 0;
static int pending_state = -1;   // candidate next state to confirm
...
static float last_peaks[N_LAST_SAVED] = {0.0f};
static float last_confs[N_LAST_SAVED] = {0.0f};
...
// Model for HR from acceleration prior knowledge (in bpm)
float HR_fom_ACC_float_trust_ppg4_emp(float rms, float a, float b){
    float out = a*rms + b;
    if (out > 210.0f){
        out = 210.0f;
    }
...
void update_HR_model_from_ACC_trust_ppg4_emp(float HR_PPG, float HR_ACC, float* a, float* b, int state){
// Updating like this with a constraint
...
// Funzione sigmoid
float sigmoid_conf_trust_ppg4_emp(float x, float midpoint, float scale) {
    return 1.0f / (1.0f + exp(-scale * (x - midpoint)));
}

// Weighted average of the last peaks with their confidence
void weighted_average_with_confidence_trust_ppg4_emp(float* peaks, float* confidences, int len, float* weighted_avg, float* C_est) {
...
int main_algorithm_trust_ppg4_emp(time_delta_ms_t delta_ms, ppg_t ppg, accel_t accx, accel_t accy, accel_t accz){
    ...
    rolling_stats_addValue_float((float)acc_filtered, &stats_acc);
    float mean_acc = rolling_stats_get_mean_float(&stats_acc);
    ...
    float acc_standardized = (float)(acc_filtered - mean_acc) / std_acc;

    // ACCx signal
    rolling_stats_addValue_float((float)accx_filtered, &stats_accx);
    float mean_accx = rolling_stats_get_mean_float(&stats_accx);
    if (rolling_stats_get_variance_float(&stats_accx) < 0.0f){
        rolling_stats_reset_float(&stats_accx);
    }
    float accx_standardized = (float)(accx_filtered - mean_accx) / std_acc;

    // ACCy signa
    rolling_stats_addValue_float((float)accy_filtered, &stats_accy);
    float mean_accy = rolling_stats_get_mean_float(&stats_accy);
    if (rolling_stats_get_variance_float(&stats_accy) < 0.0f){
        rolling_stats_reset_float(&stats_accy);
    }
    float accy_standardized = (float)(accy_filtered - mean_accy) / std_acc;

    // ACCz signal
    rolling_stats_addValue_float((float)accz_filtered, &stats_accz);
    float mean_accz = rolling_stats_get_mean_float(&stats_accz);
    if (rolling_stats_get_variance_float(&stats_accz) < 0.0f){
        rolling_stats_reset_float(&stats_accz);
    }
    float accz_standardized = (float)(accz_filtered - mean_accz) / std_acc;
    ...
    AdaptFilter_multi_put_float(&NLMS_filter, accx_standardized, accy_standardized, accz_standardized, ppg_standardized);
    float out_NLMS;
    out_NLMS = AdaptFilter_multi_get_float(&NLMS_filter, MU);

    // Dumping of the signals file 
#ifdef DUMP_FILE
    if (list_signals_C_File)
    {
        if (!fprintf(list_signals_C_File, "%d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %f, %f, %f, %f, %f, %f\n", ppg, accx, accy, accx, acc_raw, ppg_filtered, accx_filtered, accy_filtered, accz_filtered, acc_filtered, ppg_standardized, accx_standardized, accy_standardized, accz_standardized, out_NLMS, std_acc_raw ))
    ...
    if ((samples_since_last_HR == WINDOW_STEP) && (!first_window)){
        ...
        // 1) classify current window -> "desired" level
        int desired;
        ...
        if (state == STATE_STATIONARY) {
            ...
            // STATIONARY -> TRANSITION immediately on first window >= 200
            if (desired != STATE_STATIONARY) {
                state = STATE_TRANSITION;
                pending_state = desired;   // <-- target motion level based on this window
                pending_count = 1;         // first consecutive non-stationary window
            } else {
                pending_state = -1;
                pending_count = 0;
            }
            ...
            // TRANSITION -> STATIONARY if drops below 200
            if (desired == STATE_STATIONARY) {
                state = STATE_STATIONARY;
                pending_state = -1;
                pending_count = 0;
            } else {
                // still >=200: keep counting consecutive windows
                pending_count++;

                // update target level to the most recent classification
                pending_state = desired;   // <-- LOW/MEDIUM/HIGH depending on std_acc_raw now

                // after 4 consecutive windows, commit to that motion level
                if (pending_count >= N_CONSEC_STATIONARY_TO_MOTION) {
                    state = pending_state; // could be LOW / MEDIUM / HIGH
                    pending_state = -1;
                    pending_count = 0;
                }
            }
            ...
            // We are in a motion state: LOW / MEDIUM / HIGH
            // Any change (up/down/to stationary) requires 2 consecutive windows.
        ...
        float rms_float;
        float cumulative_float = 0;
        for (int i = 0; i < WINDOW_LEN; i++) {
            float val_float = (float)acc_magnitude[i];
            cumulative_float += (val_float * val_float);
        }
        float mean_double = cumulative_float / WINDOW_LEN;
        rms_float = sqrt(mean_double);

        //HR-range with the acceleration prior knowledge
        float HR_ACC = HR_fom_ACC_float_trust_ppg4_emp(rms_float, a, b);

        // At this point I define a Range based on the Motion State
        //HR-range with the acceleration prior knowledge
        int lower_limit = LOW_FREQ_PHY_I;
        int higher_limit = HIGH_FREQ_PHY_I;
        if (state == STATE_MOTION_MEDIUM)
        {
            lower_limit = (int)ceil((60.0f*N_PAD) / (SAMPLING_FREQ*60.0f));
            higher_limit = (int)floor((135.0f*N_PAD) / (SAMPLING_FREQ*60.0f));
        }
        else if (state == STATE_MOTION_HIGH)
        {
            lower_limit = (int)ceil((100.0f*N_PAD) / (SAMPLING_FREQ*60.0f));
            higher_limit = (int)floor((180.0f*N_PAD) / (SAMPLING_FREQ*60.0f));
        }
...
if (values_win_C_File)
    {
        if (!fprintf(values_win_C_File, "%d, %f, %f, %f, %f\n", state, rms_float, HR_range_centre_i_double, low_freq_HRrange_i_double, high_freq_HRrange_i_double )) 
            puts("error writing file");
        ...
        for (int i = 0; i < WINDOW_LEN; i++) {
            int buffer_i = buffer_index_plus_fftLib_float(signal_buffer_next_i, i, WINDOW_LEN);
        ...
        float fft_magnitude[HIGH_FREQ_PHY_I] = {0.0f};                    //The first ones - until MIN_FREQ_FFT_I - are initialized to zero and they keep being zero
        ...
        float c1 = (float)(peak_power/total_power);
        ...
        float c1_sigmoid = sigmoid_conf_trust_ppg4_emp(c1, MIDPOINT_C1, SCALE_C1);
        float c2_sigmoid = sigmoid_conf_trust_ppg4_emp(c2, MIDPOINT_C2, SCALE_C2);
        ...
        // If the peak is not found, the cinfidence is zero
        if (!peak_found){
            coeff_sigmoid = 0.0f;
        }
        ...
        // Kalman Filter 1 
        kalman_HR_estimation_float2(HR_meas, rms_float, coeff_sigmoid, &HR_freq_est2, &P);

        // Kalman Filter 2 (Kalman with two measures: SENSOR FUSION) 
        kalman_HR_estimation_2measures_float2(HR_meas, (HR_ACC/60.0f), state, coeff_sigmoid, &HR_freq_est3, &P_2m);
        ...
        for (int i=0; i<N_LAST_SAVED;i++){
            if (last_confs[i] >= 0.8f){ // THRESHOLD_HIGH_CONFIDENCE
        ...
        if (high_confidence){
            ...
            float c_average;
            weighted_average_with_confidence_trust_ppg4_emp(last_peaks, last_confs, N_LAST_SAVED, &HR_freq_average, &c_average);

            // Update the model
            update_HR_model_from_ACC_trust_ppg4_emp(HR_freq_average*60, HR_ACC, &a, &b, state);
...
int trust_ppg4_emp_heartrate(time_delta_ms_t delta_ms, ppg_t ppg, accel_t accx, accel_t accy, accel_t accz){
    ...
    if(delta_ms < 60){
        HR_results[0] = main_algorithm_trust_ppg4_emp(delta_ms,ppg,accx,accy,accz);
        ...
        HR_results[0] = main_algorithm_trust_ppg4_emp(delta_ms_interp, ppg_interp, accx_interp, accy_interp, accz_interp);
        HR_results[1] = main_algorithm_trust_ppg4_emp(delta_ms,ppg,accx,accy,accz);
        ...
        HR_results[0] = main_algorithm_trust_ppg4_emp(delta_ms_interp1, ppg_interp1, accx_interp1, accy_interp1, accz_interp1);
        HR_results[1] = main_algorithm_trust_ppg4_emp(delta_ms_interp2, ppg_interp2, accx_interp2, accy_interp2, accz_interp2);
        HR_results[2] = main_algorithm_trust_ppg4_emp(delta_ms,ppg,accx,accy,accz);
