/* ----------------------------------------------------
* debug10 --> NLMS + FFT + SPECTRAL PEAK with FIXED HR range + CONFIDENCE + KALMAN (NO Sensor Fusion)
* No updating dell'ACC-HR model
* Con modello FIXED molto maggiore e aggiornato sui nuovi datiS
*/
...
// Model for HR from acceleration prior knowledge (in bpm)
float HR_fom_ACC_float_debug10(float rms, float a, float b){
    float out = a*rms + b;
    if (out > 210.0f){
        out = 210.0f;
    }
...
// Funzione sigmoid
float sigmoid_conf_debug10(float x, float midpoint, float scale) {
...
int main_algorithm_debug10(time_delta_ms_t delta_ms, ppg_t ppg, accel_t accx, accel_t accy, accel_t accz){
    ...
    if ((samples_since_last_HR == WINDOW_STEP) && (!first_window)){
        ...
        float HR_ACC = HR_fom_ACC_float_debug10(rms_float, a, b);
...
if (values_win_C_File)
    {
        ...
        float c1_sigmoid = sigmoid_conf_debug10(c1, MIDPOINT_C1, SCALE_C1);
        float c2_sigmoid = sigmoid_conf_debug10(c2, MIDPOINT_C2, SCALE_C2);
...
int debug10_heartrate(time_delta_ms_t delta_ms, ppg_t ppg, accel_t accx, accel_t accy, accel_t accz){
    ...
    if(delta_ms < 60){
        HR_results[0] = main_algorithm_debug10(delta_ms,ppg,accx,accy,accz);
        ...
        HR_results[0] = main_algorithm_debug10(delta_ms_interp, ppg_interp, accx_interp, accy_interp, accz_interp);
        HR_results[1] = main_algorithm_debug10(delta_ms,ppg,accx,accy,accz);
        ...
        HR_results[0] = main_algorithm_debug10(delta_ms_interp1, ppg_interp1, accx_interp1, accy_interp1, accz_interp1);
        HR_results[1] = main_algorithm_debug10(delta_ms_interp2, ppg_interp2, accx_interp2, accy_interp2, accz_interp2);
        HR_results[2] = main_algorithm_debug10(delta_ms,ppg,accx,accy,accz);
