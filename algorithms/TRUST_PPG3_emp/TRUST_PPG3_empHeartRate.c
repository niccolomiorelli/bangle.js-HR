/* ----------------------------------------------------
* TRUST-PPG 3 on Empatica data (Accelerometer FSR = +- 16g)
* Pipeline: 
...
* - Peak selection with motion-based range reduction: BASED ON THE ACC-TO-HR MODEL
...
* - Kalman Filter-based spectral tracking with motion-based process noise and confidence-based measurement noise: SINGLE MEASURE KALMAN FILTER
...
#include "TRUST_PPG3_empHeartRate.h"
...
// Model for HR from acceleration prior knowledge (in bpm)
float HR_fom_ACC_float_trust_ppg3_emp(float rms, float a, float b){
    float out = a*rms + b;
    if (out > 210.0f){
        out = 210.0f;
    }
...
void update_HR_model_from_ACC_trust_ppg3_emp(float HR_PPG, float HR_ACC, float* a, float* b, int state){
...
// Funzione sigmoid
float sigmoid_conf_trust_ppg3_emp(float x, float midpoint, float scale) {
    return 1.0f / (1.0f + exp(-scale * (x - midpoint)));
}

// Weighted average of the last peaks with their confidence
void weighted_average_with_confidence_trust_ppg3_emp(float* peaks, float* confidences, int len, float* weighted_avg, float* C_est) {
...
int main_algorithm_debug882(time_delta_ms_t delta_ms, ppg_t ppg, accel_t accx, accel_t accy, accel_t accz){
    ...
    if ((samples_since_last_HR == WINDOW_STEP) && (!first_window)){
        ...
        float HR_ACC = HR_fom_ACC_float_trust_ppg3_emp(rms_float, a, b);

        // At this point I define a Range based on the Motion State
        //HR-range with the acceleration prior knowledge
        int lower_limit = LOW_FREQ_PHY_I;
        int higher_limit = HIGH_FREQ_PHY_I;
        if (state == STATE_MOTION_LOW ||state == STATE_MOTION_MEDIUM)
        {
            lower_limit = (int)ceil(((HR_ACC*N_PAD) / (SAMPLING_FREQ*60.0f))) - (DELTA_HR_RANGE/2);
            higher_limit = (int)floor(((HR_ACC*N_PAD) / (SAMPLING_FREQ*60.0f))) + (DELTA_HR_RANGE/2);
...
if (values_win_C_File)
    {
        ...
        float c1_sigmoid = sigmoid_conf_trust_ppg3_emp(c1, MIDPOINT_C1, SCALE_C1);
        float c2_sigmoid = sigmoid_conf_trust_ppg3_emp(c2, MIDPOINT_C2, SCALE_C2);
        ...
        kalman_HR_estimation_2measures_float2(HR_meas, (HR_ACC/60.0f), rms_float, coeff_sigmoid, &HR_freq_est3, &P_2m);
        ...
        if (high_confidence){
            ...
            float c_average;
            weighted_average_with_confidence_debug882(last_peaks, last_confs, N_LAST_SAVED, &HR_freq_average, &c_average);

            // Update the model
            update_HR_model_from_ACC_debug882(HR_freq_average*60, HR_ACC, &a, &b, state);
...
int trust_ppg3_emp_heartrate(time_delta_ms_t delta_ms, ppg_t ppg, accel_t accx, accel_t accy, accel_t accz){
    ...
    if(delta_ms < 60){
        HR_results[0] = main_algorithm_trust_ppg3_emp(delta_ms,ppg,accx,accy,accz);
        ...
        HR_results[0] = main_algorithm_trust_ppg3_emp(delta_ms_interp, ppg_interp, accx_interp, accy_interp, accz_interp);
        HR_results[1] = main_algorithm_trust_ppg3_emp(delta_ms,ppg,accx,accy,accz);
        ...
        HR_results[0] = main_algorithm_trust_ppg3_emp(delta_ms_interp1, ppg_interp1, accx_interp1, accy_interp1, accz_interp1);
        HR_results[1] = main_algorithm_trust_ppg3_emp(delta_ms_interp2, ppg_interp2, accx_interp2, accy_interp2, accz_interp2);
        HR_results[2] = main_algorithm_trust_ppg3_emp(delta_ms,ppg,accx,accy,accz);
