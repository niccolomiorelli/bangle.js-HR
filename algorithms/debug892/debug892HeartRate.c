...
* debug892 --> NLMS + FFT + SPECTRAL PEAK with FIXED HR range (based on STD) + CONFIDENCE + KALMAN (NO Sensor Fusion)
...
// Funzione sigmoid
float sigmoid_conf_debug892(float x, float midpoint, float scale) {
...
if (values_win_C_File)
    {
        ...
        float c1_sigmoid = sigmoid_conf_debug892(c1, MIDPOINT_C1, SCALE_C1);
        float c2_sigmoid = sigmoid_conf_debug892(c2, MIDPOINT_C2, SCALE_C2);
...
int debug892_heartrate(time_delta_ms_t delta_ms, ppg_t ppg, accel_t accx, accel_t accy, accel_t accz){
    ...
    if(delta_ms < 60){
        HR_results[0] = main_algorithm_debug892(delta_ms,ppg,accx,accy,accz);
        ...
        HR_results[0] = main_algorithm_debug892(delta_ms_interp, ppg_interp, accx_interp, accy_interp, accz_interp);
        HR_results[1] = main_algorithm_debug892(delta_ms,ppg,accx,accy,accz);
        ...
        HR_results[0] = main_algorithm_debug892(delta_ms_interp1, ppg_interp1, accx_interp1, accy_interp1, accz_interp1);
        HR_results[1] = main_algorithm_debug892(delta_ms_interp2, ppg_interp2, accx_interp2, accy_interp2, accz_interp2);
        HR_results[2] = main_algorithm_debug892(delta_ms,ppg,accx,accy,accz);
