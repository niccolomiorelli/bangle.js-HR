...
// Model for HR from acceleration prior knowledge (in bpm)
float HR_fom_ACC_float_debug88(float rms, float a, float b){
    float out = a*rms + b;
    if (out > 210.0f){
        out = 210.0f;
    }
...
void update_HR_model_from_ACC_debug88(float HR_PPG, float HR_ACC, float* a, float* b, int state){
...
// Funzione sigmoid
float sigmoid_conf_debug88(float x, float midpoint, float scale) {
    return 1.0f / (1.0f + exp(-scale * (x - midpoint)));
}

// Weighted average of the last peaks with their confidence
void weighted_average_with_confidence_debug88(float* peaks, float* confidences, int len, float* weighted_avg, float* C_est) {
...
int main_algorithm_debug88(time_delta_ms_t delta_ms, ppg_t ppg, accel_t accx, accel_t accy, accel_t accz){
    ...
    if ((samples_since_last_HR == WINDOW_STEP) && (!first_window)){
        ...
        float HR_ACC = HR_fom_ACC_float_debug88(rms_float, a, b);
...
if (values_win_C_File)
    {
        ...
        float c1_sigmoid = sigmoid_conf_debug88(c1, MIDPOINT_C1, SCALE_C1);
        float c2_sigmoid = sigmoid_conf_debug88(c2, MIDPOINT_C2, SCALE_C2);
        ...
        if (high_confidence){
            ...
            float c_average;
            weighted_average_with_confidence_debug88(last_peaks, last_confs, N_LAST_SAVED, &HR_freq_average, &c_average);

            // Update the model
            update_HR_model_from_ACC_debug88(HR_freq_average*60, HR_ACC, &a, &b, state);
...
int debug88_heartrate(time_delta_ms_t delta_ms, ppg_t ppg, accel_t accx, accel_t accy, accel_t accz){
    ...
    if(delta_ms < 60){
        HR_results[0] = main_algorithm_debug88(delta_ms,ppg,accx,accy,accz);
        ...
        HR_results[0] = main_algorithm_debug88(delta_ms_interp, ppg_interp, accx_interp, accy_interp, accz_interp);
        HR_results[1] = main_algorithm_debug88(delta_ms,ppg,accx,accy,accz);
        ...
        HR_results[0] = main_algorithm_debug88(delta_ms_interp1, ppg_interp1, accx_interp1, accy_interp1, accz_interp1);
        HR_results[1] = main_algorithm_debug88(delta_ms_interp2, ppg_interp2, accx_interp2, accy_interp2, accz_interp2);
        HR_results[2] = main_algorithm_debug88(delta_ms,ppg,accx,accy,accz);
